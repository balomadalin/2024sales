    ...
    public function getProgressPercentAttribute()
{
    ...
    // Verifică dacă proiectul este în desfășurare (între start_at și due_at)
    if ($now->greaterThanOrEqualTo($startAt) && $now->lessThanOrEqualTo($dueAt)) {
        $daysPassed = $startAt->diffInDays($now);
        $totalDays = $startAt->diffInDays($dueAt);

        $progressPercent = round($daysPassed / $totalDays * 100, 1);
        return $progressPercent . ' %';
    } else {
        return 'Proiectul nu este în desfășurare';
    }
}
