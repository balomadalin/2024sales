...
@php
    $lang = $this->record?->project?->client?->language ?? 'ro';
@endphp
...
const euro = (n, locale = 'ro_RO') => {
    return n ? nDigit(n, 2, locale) + ' RON' : '-,-- RON';
};

// percent number formatting
const percent = (n, locale = 'ro_RO') => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    doc.setTextColor(colors.gray).setFont('FiraSansExtraLight').setFontSize(8)
        .text(`${config.company}, ${config.taxOffice} ,${config.street}, ${config.city}`, 10, 50)
    ...
    doc.setFontSize(10).setFont('FiraSansExtraLight').setTextColor(colors.dark)
        .text(markerReplace(label.explanation, [invoice.gross, invoice.number]), 10, 270, { maxWidth: 180 })
        .text([label.regards], 100, 285, { align: 'center' });
    // footer
